...
class Blank:
    ...
    def encryptstring(self, string, config= {}, func= False):
        ...
        if not func:
            return f'{getattr}({__import__}({bytes}({b64}).decode()), {bytes}({b64decode}).decode())({bytes}({list(b64encode(string.encode()))})).decode()'
        else:
            attrs = string.split(".")
    ...
    def encrypt1(self):
        ...
        init = [f'{var1}="{encode(code[0], "rot13")}"', f'{var2}="{code[1]}"', f'{var3}="{code[2][::-1]}"', f'{var4}="{code[3]}"']

        shuffle(init)
        init = ";".join(init)
        self.code = f'''
{init};__import__({self.encryptstring("builtins")}).exec(__import__({self.encryptstring("marshal")}).loads(__import__({self.encryptstring("base64")}).b64decode(__import__({self.encryptstring("codecs")}).decode({var1}, __import__({self.encryptstring("base64")}).b64decode("{b64encode(b'rot13').decode()}").decode())+{var2}+{var3}[::-1]+{var4})))
'''.strip().encode()
...
'''.strip().encode()
    ...
    def encrypt3(self):
        self.compress()
        data = b64encode(self.code)
        self.code = f'import base64, lzma; exec(compile(lzma.decompress(base64.b64decode({data})), "<string>", "exec"))'.encode()

    def finalize(self):
        if path.dirname(self.outpath).strip() != "":
            makedirs(path.dirname(self.outpath), exist_ok= True)
        with open(self.outpath, "w") as e:
            e.write(self.code.decode())
        print("Saved as --> " + path.realpath(self.outpath))

if __name__ == "__main__":
    parser = ArgumentParser(prog= argv[0], description= "Obfuscates python program to make it harder to read")
    ...
    if not path.isfile(sourcefile := args.FILE):
    ...
    if args.path is None:
        args.path = "Obfuscated_" + path.basename(sourcefile)
    
    with open(sourcefile) as sourcefile:
        code = sourcefile.read()
    
    Blank(code, args.path)
